...
describe('TelegramChannel', () => {
  ...
  describe('non-text messages', () => {
    it('stores photo with placeholder when no file available', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
    ...
    it('stores photo with caption and download fallback', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();

      // file_id present but getFile returns no file_path → download fails gracefully
      const ctx = createMediaCtx({
        caption: 'Look at this',
        extra: { photo: [{ file_id: 'abc123' }] },
      });
      await triggerMediaMessage('message:photo', ctx);

      expect(opts.onMessage).toHaveBeenCalledWith(
        'tg:100200300',
        expect.objectContaining({ content: '[Photo (download failed)] Look at this' }),
      );
    });
    ...
    it('stores document with fallback name when filename missing', async () => {
      ...
      const ctx = createMediaCtx({ extra: { document: {} }, messageId: 1 });
      await triggerMediaMessage('message:document', ctx);

      expect(opts.onMessage).toHaveBeenCalledWith(
        'tg:100200300',
        expect.objectContaining({ content: '[Document: file_1]' }),
      );
    });
